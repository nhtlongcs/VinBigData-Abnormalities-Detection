    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            ...
            with torch.no_grad():
                ...
                if len(preds) > 0:
                    result = val_loader.dataset.run_eval(
                        preds, opt.save_dir
                    )  # add custom metrics
                else:
                    result = [0] * 4
                log_dict_val["MAP"] = result[0]
                log_dict_val["MAPsmall"] = result[1]
                log_dict_val["MAPmedium"] = result[2]
                log_dict_val["MAPlarge"] = result[3]
